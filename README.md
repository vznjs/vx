...
<!-- bench:start — generated by packages/vx-bench/update-site.ts from results.json; do not hand-edit -->

The runner adds almost nothing to a cold build. On a 1,090-package graph
of 3,270 tasks whose ideal schedule is 3m 38s, vx finishes in 3m 46s
(+4%), Turborepo in 5m 13s (+44%) and Nx in 34m 44s (9.6× the
schedule). Fully cached runs finish in milliseconds: 510 ms across that
graph, where Turborepo takes 760ms and Nx 3.59s on the identical
workspace, and the cold build burns 35 s of CPU in vx, 73 s in
Turborepo and 114 minutes in Nx.
<!-- bench:end -->
