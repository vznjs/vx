...
1,090-package graph of 3,270 tasks whose ideal schedule is 3m 38s,
vx finishes in 3m 46s (+0:08), Turborepo in 5m 13s (+1:35) and Nx in
34m 44s (+31:06) — 8 ms of overhead per package against 88 ms and
1,712 ms, so the graph can grow and the runner stays in seconds. The cold build
burns 35 s of CPU in vx, 73 s in Turborepo and 114 minutes in Nx; a
...
vx run ci --affected      # changed since origin/HEAD, and what depends on it
