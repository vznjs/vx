...
import { extractOutputs, packArtifact, readArtifact } from '../src/cache/archive.js'
...
/** The composition the cache uses: read (validates names) then extract. */
...
describe('readArtifact — name rejections', () => {
  it('rejects a name containing a `..` segment', async () => {
    await expect(readArtifact(tarWithEntry('outputs/../evil', new Uint8Array(1)))).rejects.toThrow(
      /escape|traversal|unsafe/i,
    )
  })

  it('rejects an absolute name', async () => {
    await expect(readArtifact(tarWithEntry('/etc/passwd', new Uint8Array(1)))).rejects.toThrow(
  ...
  it('rejects backslash separators', async () => {
    ...
    await expect(
      readArtifact(tarWithEntry('outputs\\foo\\bar.txt', new Uint8Array(1))),
    ).rejects.toThrow(/windows|backslash|unsafe/i)
  })

  it('rejects a Windows drive-letter prefix', async () => {
    await expect(
      readArtifact(tarWithEntry('C:/Users/admin/evil.txt', new Uint8Array(1))),
    ).rejects.toThrow(/windows|drive|unsafe/i)
  })

  it('rejects a Windows extended-length prefix', async () => {
    await expect(readArtifact(tarWithEntry('//?/C:/evil', new Uint8Array(1)))).rejects.toThrow(
  ...
  it('rejects a truncated archive rather than serving short bytes', async () => {
    ...
    await expect(readArtifact(tar)).rejects.toThrow(/truncat/i)
  })
  ...
  it('accepts an entry whose data is fully present (control)', async () => {
    ...
    expect(entries.map((e) => [e.name, e.size])).toEqual([['outputs/whole.js', body.length]])
  })
})
