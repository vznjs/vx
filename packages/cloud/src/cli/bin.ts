...
function printHelp(): void {
  process.stdout.write(
    [
      ...
      '              `vx run` then pushes its summary there. --distribute opts the',
      '              environment into ambient distribution across an agent pool',
      '              (fails safe to a local run); --no-use skips activation.',
