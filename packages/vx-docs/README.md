...
- **The numbers** are generated: `packages/vx-bench/update-site.ts`
  ...
  the graph's size (the `#bench` kicker, the first card) from
  `results.json`, and the `n8nRows` block and the `#real` kicker's task
  count from `docs/benchmarks.md`'s n8n table; `check.site` fails when
  any drifted. Edit the generator or its sources, not those regions.
  The overhead row reads the same constants (`raceBase`, `lanes`,
