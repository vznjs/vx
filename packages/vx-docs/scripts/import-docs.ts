 ...
 * Its transforms are exported for tests/import-docs.test.ts (item 837); the
 * run happens only when this file is the entry point.
 */
...
export const GENERATED_MARK =
  '# generated by packages/vx-docs/scripts/import-docs.ts from packages/vx/docs/ — edit the source, not this file'
...
export function cleanUrlFor(srcRel: string): string {
...
export function outRelFor(srcRel: string): string {
...
function collectSources(docsDir: string): string[] {
  const sources: string[] = []
  for (const pattern of ['*.md', 'modules/*.md', 'design/*.md']) {
    for (const f of new Glob(pattern).scanSync({ cwd: docsDir })) {
...
export function buildLinkMap(sources: readonly string[]): Map<string, string> {
...
type LinkMap = ReadonlyMap<string, string>
...
function transformText(text: string, sourceDir: string, prefix: string, linkMap: LinkMap): string {
  const linked = text.replace(LINK_RE, (whole, url: string, title = '') => {
    const rewritten = resolveInternal(url, sourceDir, prefix, linkMap)
...
function transformProse(line: string, sourceDir: string, prefix: string, linkMap: LinkMap): string {
  let out = ''
  let i = 0
  while (i < line.length) {
    if (line[i] === '`') {
      ...
      if (close === -1) {
        ...
        out += transformText(line.slice(i), sourceDir, prefix, linkMap)
        break
      }
      ...
      out += transformText(line.slice(i, j), sourceDir, prefix, linkMap)
      i = j
    }
  }
  return out
...
export function transformFile(srcRel: string, content: string, linkMap: LinkMap): string {
  ...
  const body = lines.map((line) => {
    ...
    return inFence ? line : transformProse(line, sourceDir, prefix, linkMap)
  })
...
/**
 * Regenerate `outDir`'s imported pages from `docsDir`. Returns how many
 * sources it imported.
 */
export async function importDocs(docsDir = DOCS_DIR, outDir = OUT_DIR): Promise<number> {
  const sources = collectSources(docsDir)
  const linkMap = buildLinkMap(sources)
  ...
  for (const srcRel of sources) {
    ...
    const transformed = transformFile(srcRel, content, linkMap)
    const outRel = outRelFor(srcRel)
  ...
  await Bun.write(path.join(outDir, 'design/index.md'), designIndex(designFiles))
  return sources.length
}

if (import.meta.main) {
  const count = await importDocs()
  console.log(`imported ${count} docs → ${path.relative(process.cwd(), OUT_DIR)}`)
}
