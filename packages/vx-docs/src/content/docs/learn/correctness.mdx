...
trusting it. It uses one package, `app`, with one task, `build`. It
...
```mermaid
graph TD
  read["app#build reads banner.txt"] -->|declared| folded["the key folds banner.txt: an edit moves the key, and the task runs"]
...
`app#build` runs `mkdir -p dist && cat src/index.ts banner.txt > dist/out.txt`. Its inputs
...
- **Declaring is work, twice.** You list each task's inputs, and a
  ...
  task may touch ([Sandboxing tasks](../../guides/sandboxing/)).
