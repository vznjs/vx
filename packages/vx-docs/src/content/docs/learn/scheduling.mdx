...
nothing. And `app` adds `app#docs`, which builds its documentation. It
waits for nothing, and nothing waits for it. Every task has a duration, made up
...
`app#docs`. Only two can start, and which two decides how the run ends.
...
`app#docs` have none, so they tie, and the tie goes to the order they
were listed in. `app#docs` starts only at 8 seconds and ends at 18. At 18
...
ahead of it, `app#docs` 10, and each lint 1 or 2. So `app#docs` starts
...
- On 2 workers, make `app#docs` take 20 seconds. Now the work sets the
  bound, not the chain: 58 seconds over two workers is 29. Learned
  durations reaches it, and tasks waiting finishes at 30.
- Tick "No history" for `app#docs`, then for `ui#lint` instead, and
...
If `app#docs` is new, the median guess is 2 seconds. It starts late and
...
export default defineWorkspace({
  concurrency: 4,
  plugins: [scheduleHistoryPlugin({ assume: { 'app#docs': 10_000 } })],
})
```
