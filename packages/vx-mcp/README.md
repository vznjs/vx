...
| `getRunHistory`    | "Which tasks have I been running, and how fast?" — recent runs plus per-task p50 / p99 / success rate / hit rate / failure mode (flaky only on a real nondeterminism signal), and the largest peak RSS and CPU parallelism the executions showed (`maxPeakRssBytes`, `maxCpuParallelism` — what `@vzn/vx-schedule-history` reserves from; a task lighter than vx itself shows no peak). Filters: `project`, `task`, `limit`. |
