...
function countVx(file: string): Map<string, number> {
  ...
  for (const l of lines) {
    ...
    const ret = /\) = (\d+)$/.exec(l)
    if ((name === 'vfork' || name === 'clone' || name === 'clone3' || name === 'fork') && ret)
      spawnedBy.set(ret[1]!, pid!)
  }
...
const a = countVx(after)
const b = before === undefined ? undefined : countVx(before)
