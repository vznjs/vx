...
// cache block and a group task. Each `vx.config.mjs` on disk is the
// config object below serialised, so the CLI evaluates exactly the object
// the playground is handed.
...
const config = (name: string): string =>
  `export default ${JSON.stringify(CONFIGS[name], null, 2)}\n`
...
export const FILES: Record<string, string> = {
  ...
  'packages/utils/vx.config.mjs': config('@pg/utils'),
  ...
  'packages/core/vx.config.mjs': config('@pg/core'),
  ...
  'packages/ui/vx.config.mjs': config('@pg/ui'),
