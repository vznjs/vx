...
Fully cached runs finish in milliseconds: ${Math.round(vx.warmNoRestore)} ms across a
${d.packages.toLocaleString('en-US')}-package graph of ${nodes.toLocaleString('en-US')} tasks, where Turborepo takes ${disp(turbo.warmNoRestore)} and Nx
${disp(nx.warmNoRestore)} on the identical workspace, and a cold build of that graph burns
${Math.round(vx.freshCpu / 1000)} s of CPU in vx, ${Math.round(turbo.freshCpu / 1000)} s in Turborepo and ${Math.round(nx.freshCpu / 60_000)} minutes in Nx.
<!-- bench:end -->`
