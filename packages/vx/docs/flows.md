...
filter drops the segments `node_modules`, `.git` and `.vx`, the
suffixes `.tsbuildinfo` and a trailing `~`, and the run's resolved
cache dir (which `.vx` covers only until `cacheDir` relocates it).
