...
|                                          | `src/cache/{layer,policy,zstd,file-hashes,config-evals,output-index,run-history,key-fold}.ts` — the slices `Cache` composes (cache.md § Files); `src/cache/{archive,tar-stream}.ts` — pack / scan / extract (cache.md, caching.md § Storage layout). |
