...
| anything else                        | `cli/plugin-commands.ts:resolvePluginCommand` — the workspace's plugins are asked, in order, for a `commands` entry (`vx mcp` from `@vzn/vx-mcp` is one); a verb resolving a non-integer fails naming the plugin; nothing found → one line: `unknown command`, a near-miss hint when one exists, why plugin verbs could not be looked up when the workspace file failed to load, and a `vx help` pointer; exit 1 |
