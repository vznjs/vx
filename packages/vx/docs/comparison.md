...
| Lockfile-aware invalidation (only the projects a dependency change reaches) | (whole lockfile in the global hash)                  | yes — pruned lockfile per project, in the daemon's graph | (whole lockfile)                   | **`@vzn/vx-pnpm`** — per-project closure digest, memoised by lockfile hash; `--affected` follows it |
