...
| one-at-a-time task         | —                                                            | `parallelism: false`                            | —                                   | `exec.resources` with `cpus` at or above the machine's cores runs alone; or `--concurrency 1` |
...
| Migration generator from other runners                                      | —                                                    | —                                                        | —                                  | `bunx @vzn/vx-migrate` (turbo.json / Nx graph → vx.config.ts)                                                                   |
...
| Remote cache wire        | Vercel `/v8/artifacts/` (HMAC, pre-signed) | Nx Cloud or plugin     | —                            | plugin-driven: `@vzn/vx-reapi` (Bazel AC/CAS), `@vzn/vx-migrate`'s `turboCache()` (`/v8/artifacts`, self-hosted or Vercel), `@vzn/vx-nx-cache` (`/v1/cache`) |
