...
| Inferred tasks from tool configs (`vite.config` ⇒ build/serve/test)         | Nx plugins    | SHIPPED 2026-09-10 as `@vzn/vx-infer`: `vite()`, `vitest()`, `next()`, `tsc()` on the `project` stage, and `scripts()` for every `package.json` script (uncached — a script says nothing about what it reads). Not core.                                                                                                                                                                                      |
