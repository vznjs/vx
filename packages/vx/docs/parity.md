...
| `"cache": false`                                                   | no `cache` block (`noCache: true` in `--summarize`)                                         | `tests/no-cache-word.test.ts`, `tests/run-artifacts.test.ts`                           |
...
| `turbo prune`                                                      | **≠** removed 2026-09-11 (was `@vzn/vx-prune`): copy the workspace and `--filter` the build |                                                                                        |
| `--summarize` + Vercel Remote Cache                                | `turboCache()` in `@vzn/vx-migrate` (same wire, self-hosted or Vercel)                      | `packages/vx-migrate/tests/turbo-cache.test.ts`                                        |
...
| `nx affected` (changed **and dependents**)                                         | the same: `--affected` is changed and dependents; `--filter '[<ref>]'` is the changed set | `tests/affected-dependents.test.ts`                          |
...
| `nx reset`                                                                         | `vx cache prune`                                                                          | `tests/cache.test.ts`, `tests/cli.test.ts`                   |
