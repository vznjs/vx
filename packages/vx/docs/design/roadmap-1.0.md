...
| W2  | ~~Caching from first principles, with a key calculator.~~ DONE in item 684: `learn/caching.mdx` with `KeyCalculator.astro`, a model of the key fold that `tests/key-model-core.test.ts` holds to real vx runs.                                                                  | M    |
| W3  | ~~Correctness: trusting the cache, with a stale-hit demo.~~ DONE in item 688: `learn/correctness.mdx` with `StaleHit.astro`, held to real vx runs and core's report formatter by `tests/learn-correctness.test.ts`.                                                             | S–M  |
| W4  | ~~Scheduling, with the item 669 simulator running in the browser.~~ DONE in item 685: `learn/scheduling.mdx` with `SchedulerSim.astro`, whose element bundles vx-bench's simulator over core's and the plugin's ranking files.                                                  | M    |
| W5  | ~~The architecture as a pipeline with seams, with a pipeline explorer.~~ DONE in item 686: `learn/architecture.mdx` with `PipelineExplorer.astro` over `demos/model/pipeline.ts`, pinned to `PLUGIN_HOOKS` both ways.                                                           | M    |
| W6  | ~~Extending vx: worked plugins that the site's tests type-check.~~ DONE in item 686: `learn/extending.mdx`, five plugins from `src/examples/`, each type-checked and held to the stage it lights.                                                                               | M    |
| W7  | ~~Choosing between vx, Turbo, Nx and Bazel: design choices and their costs, not a feature table.~~ DONE in item 689: `learn/choosing.mdx` with `ChoosingMatrix.astro` over `demos/model/choosing.ts`, every other-tool cell linked to that tool's docs.                         | M    |
| W8  | ~~A landing page that leads with the problem and the ideas, then the numbers.~~ DONE in item 709: the hero states the problem, three idea sections link their Learn pages, the numbers follow, pinned by `tests/landing.test.ts`.                                               | S    |
| W9  | ~~The playground: the real planner (graph, keys, scheduler) running in the browser over a workspace the reader edits. Starts with a spike.~~ DONE in items 676–703: `learn/playground`, core's planner bundled unchanged and held to `vx run --dry=json` by core's parity rows. | L    |
| W10 | ~~Labs: break a build on purpose (a stale hit, an undeclared input, an output collision) and see what catches it.~~ DONE in item 704: `learn/labs`, three playground labs held to the CLI, and a bad order on the scheduling simulator.                                         | M    |
| W11 | ~~Checkpoints: one question per page, checked against the live model.~~ DONE in item 705 for the four pages whose questions the planner answers; scheduling, choosing, architecture and extending keep a static `<details>`.                                                    | S    |
