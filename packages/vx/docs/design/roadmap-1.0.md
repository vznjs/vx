...
| #   | Item                                                                                                                                                                                           | Who       | Size |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---- |
| 1.1 | Publish the plugin packages. Extend `npm.yml` to publish the seven plugins with provenance, at the core's version, core first. Add a law that every non-private package is in the publish set. | agent     | S    |
| 1.2 | Make the docs match. Every "published" or `bunx @vzn/…` line is true on the day of the release, pinned by a test that reads the publish list.                                                  | agent     | S    |
| 1.3 | Refresh the 0.1.0 notes with items 624–654.                                                                                                                                                    | agent     | S    |
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| 2.1 | **Automatic cache eviction during a run.** A size or age cap from `vx.workspace.ts`, applied at run end. The `vx cache prune` flags exist but nothing applies them unattended.                                                                                                                                                                        | The one open parity row (`comparison.md` "maybe worth adding" #11). Turbo and Nx both bound their cache.                                                                                        | M                                             |
| 2.2 | **Streaming remote seam (D2).** `RemoteCacheLayer.get/put` stream a `Blob` or `ReadableStream` instead of a whole buffer. `vx-reapi` gets a streaming digest and chunked upload.                                                                                                                                                                      | It is a breaking change to the plugin API. Deferred "until a workspace uploads > 100 MiB", but a seam that will change cannot be frozen. It must land before milestone 3, demand signal or not. | L                                             |
| 2.3 | **Close the stale parity ledgers.** `design/turbo-nx-parity-2026-07.md` has 19 entries with no status, and `design/turbo-nx-test-gaps.md` says "~57 gaps". Mark each fixed (with its item) or open. Two look genuinely open and need a decision: Nx H7 (live-evaluating configs without a frozen lock) and Nx L2 (bare-name filter segment matching). | "Parity complete" has to be a checkable claim, not a memory.                                                                                                                                    | S–M                                           |
