...
// Root writes anywhere, so the case skips there — unless VX_REQUIRE_NONROOT
// says CI expected to run it (see helpers/nonroot-gate.ts).
describe.skipIf(skipAsRoot('a cache directory this user cannot write into'))(
  'a cache directory this user cannot write into',
  () => {
    let root: string
    let ro: string
    beforeEach(async () => {
