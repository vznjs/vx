...
/**
 * Two readers feed the same staging core: the in-memory composition
 * (ingest, and what `readArtifact` callers see) and the streaming one the
 * cache restores through. Every restore case below runs against BOTH — a
 * defense that holds for one reader and not the other is a hole.
 */
type Restore = (bytes: Uint8Array, dest: string, wsDest?: string) => Promise<void>
const READERS: Array<[string, Restore]> = [
  [
    'in-memory reader',
    async (bytes, dest, wsDest) => {
      await extractOutputs(await readArtifact(bytes), dest, wsDest)
    },
  ],
  [
    'streaming reader',
    async (bytes, dest, wsDest) => {
      // Odd-sized chunks, so entry boundaries never line up with them.
      const stream = new ReadableStream<Uint8Array>({
        start(c) {
          for (let i = 0; i < bytes.byteLength; i += 700) c.enqueue(bytes.subarray(i, i + 700))
          c.close()
        },
      })
      await extractArtifactStream(stream, dest, wsDest)
    },
  ],
]
let restore: Restore = READERS[0]![1]

describe.each(READERS)('%s', (_name, reader) => {
  beforeEach(() => {
    restore = reader
  })
  ...
  describe('archive restore — non-regular entries are never materialized', () => {
    ...
    for (const { label, typeFlag, linkname } of SMUGGLED) {
      it(`ignores a ${label} (typeflag '${typeFlag}')`, async () => {
        ...
        expect(await readArtifact(tar)).toEqual([])
        await restore(tar, dest)
  ...
  describe('name rejections', () => {
    ...
    it('rejects a name containing a `..` segment', async () => {
      await expect(
        restore(tarWithEntry('outputs/../evil', new Uint8Array(1)), dest),
      ).rejects.toThrow(/escape|traversal|unsafe/i)
    })
...
describe('packArtifact → readArtifact → extractOutputs round trip', () => {
  ...
  it('round-trips a path longer than 100 bytes and a >100-byte component', async () => {
    ...
    const entries = await readArtifact(bytes)
    expect(entries.map((e) => e.name).sort()).toEqual([`outputs/${deepRel}`, 'stdout'].sort())

    await extractOutputs(entries, dest)
