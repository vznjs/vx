...
/** A site page: Markdown, or MDX when the page holds a component (a Learn widget). */
const PAGE = /\.mdx?$/
...
describe('every core file path the guides and CLAUDE.md name exists', () => {
  it('src/, tests/ and docs/ paths resolve under packages/vx', () => {
    const files = [
      ...walk(path.join(repo, 'packages', 'vx-docs', 'src', 'content', 'docs', 'guides'), PAGE),
...
function sitePages(): string[] {
  // Read defensively: this tree is @vzn/vx-docs#build's OUTPUT, and that
  // task runs beside this one in `vx run ci --all`. A page that vanishes
  // between the walk and the read is one the import script is rewriting —
  // its source is pinned by the safe half either way, and the alternative
  // (an ENOENT stack under an unrelated row) reads as a broken test, which
  // is how it surfaced on 2026-09-20.
  return walk(path.join(repo, 'packages', 'vx-docs', 'src', 'content', 'docs'), PAGE).filter(
    (p) => {
...
/** The file's text, or undefined if it is gone — see `sitePages`. */
...
function siteSource(urlPath: string): string | undefined {
  ...
  for (const candidate of [
    ...
    path.join(CONTENT, `${page}.mdx`),
    path.join(CONTENT, page, 'index.md'),
    path.join(CONTENT, page, 'index.mdx'),
  ]) {
...
describe('every relative link on the site pages resolves', () => {
  it('`](../page/)` is a page, an asset link a file, and `#anchor` a heading by its rendered id', () => {
    ...
    for (const file of sitePages()) {
      ...
      const urlDir = `/${rel.replace(/(^|\/)index\.mdx?$/, '').replace(/\.mdx?$/, '/')}`
      const text = readIfPresent(file)
      if (text === undefined) continue
      for (const { target, anchor } of proseLinks(text)) {
        ...
        if (anchor === undefined || !PAGE.test(source)) continue
