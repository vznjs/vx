...
| Cache             | `Cache`, `LayeredCache`, `RemoteCache`, `GitFilesCache`, `resolveInputs`, `resolveOutputs`; types `CacheLayer`, `RunRecord`, `InvocationRecord` (the CAS seam — `CASBackend`/`Digest` — is internal until the artifact store lands)                      |
