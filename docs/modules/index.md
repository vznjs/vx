...
| Metrics queries   | `listRuns`, `getRun`, `getInvocation`, `listInvocations`, `getHistory`, `cacheKeyDiff`, `whyDidThisRerunQuery`, `explainCacheKeyQuery`, `getCacheStatsSql`, `listProjects` — what `vx why` / `vx last` / `vx info` / `vx mcp` read                                                                                                                                            |
