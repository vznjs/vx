...
slowest task is _not_ the most-blocking task. Common in real
...
historical flakiness gets _auto-retried once_ on transient failure
...
For _local_ predictions, the local `cache.db` is sufficient. For
_team-wide_ predictions ("most other people see this test as slow"),
...
mis-prediction only changes _order_ and _priority_, not WHAT runs.
...
- **History invalidation.** A task whose command changed yesterday
  has irrelevant history. We weight by _task config hash similarity_
