...
its dependencies to finish *running* before it is probed and restored —
...
The user: *"it should not wait for compute if it can be restored ASAP —
why does it wait?"*
...
1. **Input dependency** — a downstream task whose `cache.inputs.files`
   ...
   spells it out. For these tasks the key is *preliminary* until the
   upstream runs — you cannot probe the cache for a key you can't yet
   derive.

2. **Output ordering / cleanliness** — `executeCachedTask` *cleans then
   restores* the declared outputs (`execute-task.ts:297-302`). If an
...
computes) and which is a **confirmed local hit**, *neither* reason
...
*ordering* edge with no data behind it — and that's the edge we can
relax.
...
task; (b) that task being a *local* hit this run; (c) at least one of its
deps actually *running* (a miss, a no-cache task, or a slow exec) for
...
- **Always-run upstream feeding a cached downstream.** The user's
  ...
  step that a cached `build` depends on for *ordering* but not for
  *inputs*. Common in docs/site pipelines and any "prep then build"
...
- **Warm all-hit runs.** If *every* task is a local hit, deps finish in
...
*latency* improvement on mixed graphs, not a throughput improvement on
warm graphs. The engineering cost is concentrated entirely in *not
regressing* the warm path. That asymmetry (small, narrow win vs. a
...
are *pre-resolved*: it skips `execute()` for them and treats them as
...
for no benefit. A keeps the scheduler's invariant intact by *removing*
...
Build **Option A**, gated and structured to be *provably free* on the
...
1. **Returns immediately** with an empty `resolved` and a settled `done`
   when the short-circuit cannot help — *this is the warm-path / opt-out
   guard, see §gating*. The decision must be cheap (a flag check), not a
   pass over the graph.
...
2. Otherwise derives stable keys **exactly** as `deriveStableKeys` in
   ...
   reuse. *Factor `deriveStableKeys` + its helpers out of
   `remote-prefetch.ts` into a shared file* (e.g.
...
3. For each stable, cache-enabled, **local-read-enabled** task, probe
   `cache.get(hash, ctx)` and run the *same* skip-restore /
...
This is the crux — the prior `classify.ts` regression was *entirely*
...
for these nodes, there is *no* second probe by construction. The restore
already happened in the background; the scheduler just records the
outcome. This is strictly *less* work than today (one probe + one restore
...
`failedDep` skip check (`scheduler.ts:240`) still runs *before* this — so
...
log suggested. It's *more* invasive: `executeCachedTask` would need an
...
— the restore *and* outcome construction happen in one place
...
Note the **timing constraint**: `resolved` must be populated *before*
...
but the *scheduler* must see the entry when it gets to that node. Two
sub-options:
...
- **1a (simplest, recommended first slice):** `await` the probe phase
  ...
  with the *decision* (hit + which to restore); kick the restores off in
...
- **1b (more overlap, more complexity):** don't await the probe phase;
  ...
  to *also* handle "node became ready before its `resolved` entry
...
this is a *local* optimization and applies to local-only runs (the
common `vx run` case). That is the whole point — the user's repro is
local-only.

**The warm-path question.** A warm all-hit run *would* qualify under the
above (it has deps, local reads on). To keep it free we must ensure the
probe phase is not *additional* work. It isn't, under 1a, *if* we accept
...
*nothing* (every node is pre-resolved). So total work = N probes + N
restores, same as today, just reordered and parallelized. The risk is
*not* double work (there is none — `execute()` is skipped); the risk is
...
The one *measurable* risk in the warm case is the bounded-pool overhead
...
*when* a local probe/restore fires, never the key derivation
...
- **Cross-project upstream** → its project-relative outputs land in
  *its* dir, never D's. D's project-relative outputs land in D's dir.
  No overlap by the boundary rule.
- **Workspace-outputs (the no-boundary escape hatch)** → root-anchored
  outputs *can* land anywhere. The gate already marks a task unstable if
  it reads `workspaceFiles` and an upstream writes `workspaceFiles`. But
  there's a residual case the *remote* prefetch tolerates because remote
  ...
  (restore).** So we need one *extra* guard the remote path doesn't:
...
*before* its dep's outcome is known — so it would be reported `cache-hit`
even if its dep later fails.

**Is that correct?** Decompose:

1. **Is the restored artifact valid?** Yes, unconditionally. The key
   folds the upstream's *input* hash (pure-input transitive hashing —
   `upstream.ts`, the v22 model), never the upstream's *success* or
   *output*. A cached hit for key K means "for this exact input
   fingerprint, here is the recorded output." The upstream *failing this
   run* does not change D's input fingerprint (the inputs are the same
...
2. **Does vx's contract expect the dependent to be skipped?** Today's
   contract is "if a dep fails, don't *run* the dependent" — because
   running it might consume the dep's (missing/broken) output, or because
   continuing is pointless when an upstream is broken. The *intent* is
...
- But **reporting** `cache-hit` for a task whose dep failed changes
  ...
  short-circuit they'd see `cache-hit`. That's a *semantic* change to
  what a failed run looks like, even though it's arguably *more* correct
...
if (failedDep) { finishOne(id, { ...skipped }); continue }   // unchanged, runs FIRST
const pre = preResolved?.get(id)
if (pre) { onStart?.(node); finishOne(id, pre); continue }   // new, runs AFTER
```

Because the scheduler only reaches D *after* all D's deps have completed
(D is still in the normal `pending`/`ready` flow for *scheduling
order* — we do NOT relax that; we only skip `execute()`), the
`failedDep` check sees the real dep outcomes. So:

- **Dep failed** → D is reported `skipped`, identical to today. The
  background restore *already wrote D's outputs to disk* — a minor
  ...
  **Recommendation:** keep it simple — the restore is harmless (valid
  ...
  defer the *restore* until the scheduler confirms no dep failed, keeping
  only the *probe* early — but that loses most of the overlap win and
...
the *success* path; it never changes what a failed run reports.
...
`execute()` *subprocess/probe* — the restore having been hoisted. The
"don't wait for compute" win comes from the restore overlapping the deps'
runtime, while the *reporting* still happens in dependency order. The
user's 76ms→overlap win is preserved (the restore I/O overlaps U's run);
D's *outcome line* still prints after U's, which is fine — the work was
done early.

> **Re-read of the win:** D's restore happens at t≈1 (background,
> overlapped with U). D's outcome is *recorded* at t=76 when the schedule
> reaches it. So the user-visible *latency* win is "the restore I/O
...
- If it's a **dependency** (not requested/surfaced) → it never streamed
  ...
  *reporting* it in schedule order (per the skipped-decision above) means
...
- If it's the **requested** task itself (focused, single) → today it
  ...
  watching; the overlap win on the *requested* task is marginal (it's
  ...
  **exclude `node.requested || node.surfaced` from short-circuit.** This
  ...
  whole class of ordering risk. The win is on the *dependency* layers
  ...
  > tasks, the repro's *specific* task isn't short-circuited. BUT the
  ...
  > atomic block (`logger.ts:447-481`) and *can* be short-circuited
  ...
  > the frame. If the user's real pain is a *wide* set of cached builds
...
- **`--force`** (reads off, writes on): `localRead` false → no-op. Tasks
  re-execute and refresh the cache, as today. Correct — `--force` *means*
...
**inverse**: it's local-first and runs for *both* local-only and layered
...
- **Layered run:** remote-prefetch already fires (warming local from
  ...
  short-circuit probes *before* remote-prefetch has ingested, it sees a
  ...
  inflight de-dup). So a stable task that's only a *remote* hit won't be
  ...
  Short-circuit's added value is purely the *local* hit's restore.
  ...
  **Should we unify?** Both modules derive the same stable keys (the
  ...
  shared `stable-keys.ts`, two thin callers. Merging the *derivation*
  removes the drift risk the decision log cares about; merging the
  *actions* would entangle two different correctness stories.
...
switch from §7 makes A/B a one-flag flip — measure the *same binary*
...
1. **Warm all-hit (the landmine).** 1090 pkgs, all tasks cacheable, run
   ...
   *Sub-measurement:* time the pre-schedule probe phase in isolation
...
3. **The target case — slow upstream, cached downstream.** A generated
   ...
   scale with the *width* of the cached layer behind the slow prep (many
...
- **Short-circuiting the *requested single-focused* task** — its live
...
- **Reuses proven machinery.** The stable-key gate, key-derivation
  reuse, bounded pool, and never-fail lifecycle are *already shipped* in
...
- **Provably free on the warm path by construction** — the short-circuit
  *removes* nodes from `execute()` rather than adding a second probe, so
...
- **No CACHE_VERSION / SCHEMA bump** — purely changes *when* the local
...
The single-task win is ~2ms and the user's specific repro is a *focused
single requested task* whose live frame we'd rather not disturb. The
