# Decision log archive (2026-05 → 2026-08)

The full, unabridged decision log that lived in `CLAUDE.md` until it grew to
~1 MB. It is preserved here verbatim — nothing was dropped — because the
value of these entries is in their detail: the measured numbers, the refuted
hypotheses, the probe mistakes, and the reasons a fix took the shape it did.

`CLAUDE.md` now carries a compacted version: the standing rules distilled out
of this history, the live invariants, the open items, and the recent entries.
**Grep here by date or symbol when you need the full story behind a decision**
(`grep -n '2026-07-26' docs/design/decision-log-archive.md`), and keep
appending new entries to `CLAUDE.md`, not to this file.

Entries are newest-first, the same order they were written in.
